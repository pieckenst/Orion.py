...
from ..model import InteractionOptionType, InteractionPermissionType


async def add_slash_command(bot_id, bot_token: str, guild_id, cmd_name: str, description: str, options: list = None):
    """
...
def create_option(name: str, description: str, option_type: typing.Union[int, type], required: bool, choices: list = None) -> dict:
    """
    ...
    if not isinstance(option_type, int) or isinstance(
        ...
        option_type = InteractionOptionType.from_type(original_type)
...
def generate_options(function: Callable, description: str = "No description.", connector: dict = None) -> list:
    ...
    for param in params:
        ...
        option_type = (
            InteractionOptionType.from_type(param.annotation) or InteractionOptionType.STRING
        )
...
def create_permission(id: int, id_type: typing.Union[int, InteractionPermissionType], permission: bool):
    """
    Create a single command permission.
    :param id: Target id to apply the permission on.
    :param id_type: Type of the id, :class:`..model.InteractionPermissionsType`.
    ...
    if not (
        ...
        id_type = InteractionPermissionType.from_type(original_type)
        if id_type is None:
            raise IncorrectType(
...
def create_multi_ids_permission(ids: typing.List[int], id_type: typing.Union[int, InteractionPermissionType], permission: bool):
    """
...
def generate_permissions(allowed_roles: typing.List[int] = None, allowed_users: typing.List[int] = None, disallowed_roles: typing.List[int] = None, disallowed_users: typing.List[int] = None):
    """
    ...
    if allowed_roles:
        permissions.extend(
            create_multi_ids_permission(allowed_roles, InteractionPermissionType.ROLE, True)
        )
    if allowed_users:
        permissions.extend(
            create_multi_ids_permission(allowed_users, InteractionPermissionType.USER, True)
        )
    if disallowed_roles:
        permissions.extend(
            create_multi_ids_permission(disallowed_roles, InteractionPermissionType.ROLE, False)
        )
    if disallowed_users:
        permissions.extend(
            create_multi_ids_permission(disallowed_users, InteractionPermissionType.USER, False)
        )

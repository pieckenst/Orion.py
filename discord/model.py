...
class InteractionOptionType(IntEnum):
    ...
    def from_type(cls, t: type):
        """
        Get a specific InteractionOptionType from a type (or object).
        :param t: The type or object to get a InteractionOptionType for.
        :return: :class:`.model.InteractionOptionType` or ``None``
        """
...
class InteractionMessage(ComponentMessage):
    """discord.py's :class:`discord.Message` but overridden ``edit`` and ``delete`` to work for slash command."""

    def __init__(self, *, state, channel, data, _http: http.InteractionRequest, interaction_token):
        super().__init__(state=state, channel=channel, data=data)
        self._http = _http
        self.__interaction_token = interaction_token

    async def _interaction_edit(self, **fields):
        """
...
class PermissionData:
    ...
    :ivar type: The ``InteractionPermissionsType`` type of this permission.
...
class InteractionPermissionType(IntEnum):
    """
